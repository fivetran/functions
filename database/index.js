...
exports.handler =  (request, context, callback) => {
  ...
  pool.getConnection(function(err, connection) {
    // Use the connection
    var query = "";
    if(request.state.employee === undefined || request.state.employee === null) {
    ...
    connection.query(query, function (error, results, fields) {
      ...
      if (error) callback(error);
        ...
        for (i in results) {
          ...
          // 'isDeleted' defines if teh column has been deleted or not
          var a = { "column_1": results[i].column_1, "column_2": results[i].column_2, "column_3": results[i].column_3,"isDeleted":results[i].isDeleted,"last_modified":results[i].last_modified }
          console.log(a);
          if(results[i].isDeleted===0){
          insertTransactions.push(a);
          }
          else{
            deleteTransactions.push(a);
          }
         cursor = a.last_modified
        }
